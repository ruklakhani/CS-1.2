...
class Listogram:
    ...
    def build_listogram(self): 
        ...
        for word in self.word_list:
            index = self.get_index(word, histogram)
            if index is None:
    ...
    def get_index(self, word, list_histogram):
        ...
        for i in range(0, len(list_histogram)):
            if list_histogram[i][0] == word:
                return i
        return None
    ...
    def frequency(self, word):
        ...
        index = self.get_index(word, self.list_histogram)
        if index is None:
            return 0
        else:
    ...
    def sample(self):
        ...
        total = 0
        for pair in self.list_histogram:
            total += pair[1] 
            if total >= rand_num:
                return pair[0]
